...
(Group E).
**Origin:** the same electron–N₂ ²Π_g shape-resonance model surface as
...
This model is a **given testbed** — the same fixed-nuclei ²Π_g resonance potential
...
2. **Hamiltonian** (`hamiltonian2d.build_h2d`):
   `H_2D = -(1/2) d²/dr² - (1/(2*MU)) d²/dR² + v0(R) + l(l+1)/(2r²) + V_int(r,R)`,
   `l = 2` fixed (`PARAMS["impulsemomentum"]`), assembled via `qscat.dvr.hamiltonian_nd`.
3. **Why `V_int` excludes the channel potentials.** `V_int(r,R) = -lambda(R)
   exp(-alpha_c r²)` is the *only* term that vanishes as `r -> infinity`; it is the sole
   perturbation relative to the entrance channel and is what drives the Lippmann-
   Schwinger equation and appears in the T-matrix vertex. `v0(R)` (the neutral
   molecule's own potential) and the centrifugal term `l(l+1)/(2r²)` are **channel**
...
1. **Zero-driving (free-particle) limit.** With the driving term forced to zero
   (`H_2D` still carries the full interaction; only the RHS vanishes), `Psi_sc = 0`
   identically and `T = 0` exactly — machine-precision, not approximate. **PASS.**
2. **First-Born limit — the strongest single check.** As `lam_scale -> 0` (a device
   that scales *only* the driving/vertex `V_int` used in the Lippmann-Schwinger RHS and
   T-matrix, never `H_2D`'s own propagator — see `ve_cross_section_2d`'s docstring),
   `Psi_sc -> 0` so `T -> <Phi_f|V_int|Psi_i>`, the first-Born amplitude, computable
   ...
   `O(lam_scale)` cross term in an *exact* (not perturbative) quadratic
   `T(lam) = lam*T1 + lam^2*T2`. This single test simultaneously pins normalization,
   masking, the DVR coefficient convention, and the T-matrix construction.
3. **σ ∝ λ² scaling.** Independently, the ratio `sigma(2*lam_scale)/sigma(lam_scale)`
   ...
   `lam_scale -> 0`, confirming `sigma ∝ lambda²` (i.e. `T` scales linearly in the
   coupling, exponent 1, and `sigma = |T|²` picks up exponent 2) directly, not just via
...
4. **S-matrix reciprocity and unitarity (flux conservation).** At `e_tot = -0.727` Ha
   ...
   **complementary, not redundant**: reciprocity, `max|S - S^T| = 3.25e-19`
   (`tol < 1e-14`) — an exact algebraic consequence of `H_2D = H_2D^T` under ECS, so it
   ...
   factor `c` leaves `S` symmetric either way); and unitarity, `max|S^dagger S - I| =
...
**θ-independence — the sharpest ECS check available.** At the rich `BASELINE` grid,
θ = 25°/30°/35°/40° differ by only 7.8e-10/6.1e-10/3.5e-10 relative: effectively exact
...
**separately, directly at the cheap `WORKING_GRID` settings** (`n_complex=5`), θ = 25°
...
θ = 35° (which also happens to be eMoScat's own undocumented choice) rather than the
...
for θ-robustness at this coarse `n_complex`), but the 98-bohr electronic box is
...
| E (Ha) | v' | σ_exact (bohr²) | σ_LCP (bohr²) | σ_Houfek (bohr²) | exact/Houfek (gate) | LCP/exact (V5) | LCP/Houfek | gated |
|---|---|---|---|---|---|---|---|---|
...
`|exact/Houfek - 1|` at the 4 GATED anchors: 1.149e-06, 8.430e-06, 1.665e-05 (E=0.2
...
Houfek than the LCP is (e.g. E=0.2, v'=1: exact deviates 1.1e-06, LCP deviates 0.555).
...
the resonance region) and **1.010** at E=0.1 Ha, v'=1 (near-unity, close to the ²Π_g
...
solution's nuclear density `rho(R) = sum_r |Psi(r,R)|^2` (electronic tail masked before
the sum, nuclear tail masked after) at `E = 0.2` Ha, `v_init = 0`, and compares it to
the LCP's own driven-equation solution `|xi(R)|^2` (both on the identical `WORKING_GRID`
nuclear axis and vibrational states):

| | centroid ⟨R⟩ (bohr) | RMS width (bohr) |
|---|---|---|
...
ripples visible in both curves near R≈1.95 and R≈2.10 bohr are **not** physical
...
- `projects/n2_2d_cross_section/test_channels.py`, `test_hamiltonian2d.py`,
  ...
  free-particle and first-Born limits, λ² scaling, S-matrix reciprocity/unitarity —
  **all PASS**, independent of any reference data.
- `projects/n2_2d_cross_section/test_convergence.py` (`@pytest.mark.slow`):
  θ-independence and refinement-stability at `WORKING_GRID`, tolerance `1e-5` (measured
