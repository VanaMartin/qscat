...
def _smooth_update(s: SmoothR | TailR, prefix: str, upd: Mapping[str, float]) -> SmoothR | TailR:
    ...
    return replace(s, coeffs=tuple(coeffs), f_inf=kw["f_inf"])


def _shell_update(s: SmoothR, upd: Mapping[str, float]) -> SmoothR:
    out = _smooth_update(s, "shell", upd)
    assert isinstance(out, SmoothR)
    return out
...
def with_params(
    ...
    return replace(
        ...
        shell=None if model.shell is None else _shell_update(model.shell, updates),
