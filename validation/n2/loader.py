...
def integrity_checks(path: Path = DATA_PATH) -> list[tuple[str, bool, str]]:
    ...
    out.append(
        (
            "C1 shape 400x32",
            d.n_energies == 400 and d.n_channels == 31,
            f"{d.n_energies} energies, {d.n_channels} channels",
        )
    )
    inc = bool(np.all(np.diff(d.energy) > 0))
    out.append(
        ("C2 energy strictly increasing (Ha)", inc, f"[{d.energy[0]:.4g}, {d.energy[-1]:.4g}]")
    )
    out.append(
        (
            "C3 cross sections non-negative (bohr^2)",
            bool(np.all(d.sigma >= 0.0)),
