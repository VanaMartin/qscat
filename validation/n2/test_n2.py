...
def test_green_groups_pass_pending_never_fail():
    ...
    # Every Group A, Group C-integrity, Group B, GATED Group C5, and Group D
    # (time-dependent) check must PASS; nothing may FAIL. NOTE rows
    # (DOCUMENTED-LIMITED C5 anchors) are informational, not failures.
    assert not any(c[2] == "FAIL" for c in checks), [c for c in checks if c[2] == "FAIL"]
    assert any(c[2] == "PASS" for c in checks)
    assert not any(c[2] == "PENDING" for c in checks)  # Group D now wired: 0 PENDING
