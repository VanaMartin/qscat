...
harness WITHOUT a live in-harness propagation.

Why no live check (measured, not assumed): a full propagation at
...
longer test anything meaningful. Per the Task 7 brief's explicit fallback
rule ("if it exceeds ~60s or needs a tolerance so loose it's meaningless,
use [the NOTE] option instead"), Group F therefore reports the ALREADY-
VALIDATED Task 4/6 numbers as literal, cited constants: no propagation
runs when the harness executes this module. See
`docs/physics/n2-2d-td-cross-section.md` for the full method and
`.superpowers/sdd/task-4-report.md` / `task-6-report.md` for the
underlying runs these constants are taken from verbatim.
...
class TdExact2dResult:
    """One TD-vs-TI anchor from a PREVIOUSLY-MEASURED, validated propagation
    at `TD_WORKING_GRID` (Task 4's T-scan and Task 6's independent
    reproduction of it) -- NOT recomputed live by the harness. `rtol` is the
    tolerance the corresponding `@slow` test in
    ...
    sigma_ti: float  # bohr^2, recorded (same run's TI-oracle comparison)
    ratio_td_ti: float  # sigma_td / sigma_ti
    rtol: float  # the @slow test's gate on |ratio - 1|
    source: str  # where these numbers come from


def compute_td_exact2d_results() -> list[TdExact2dResult]:
    """The 2 validated TD-vs-TI anchors at `TD_WORKING_GRID`'s converged
    T=1500 configuration, reported as recorded constants -- see module
    docstring for why a live propagation is not run in-harness.

    Both values are reproduced independently across Task 4 (the T-scan that
    ...
    numbers to the digits shown, so restating them here is not a fresh
    ...
    return [
        TdExact2dResult(
            ...
            ratio_td_ti=0.9305,
            rtol=0.10,
            source="test_td_cross_section.py::test_v2a_td_matches_ti_at_e010 (@slow) / "
        ...
        TdExact2dResult(
            ...
            ratio_td_ti=1.1033,
            rtol=0.15,
            source="test_td_cross_section.py::test_v2a_td_matches_ti_at_e015_usable_window_edge "
