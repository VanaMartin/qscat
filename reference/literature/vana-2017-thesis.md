...
| **Fig. 3.14** — NO-like elastic / `0→1` / `0→8` VE **and the `0→DA` cross section**, TI 2-D reference vs three TD S-matrix routes vs the LCP, at `t_c = 35000`. Bottom panel: linear axis in units of **1e-9 a₀²** over 0.170–0.200 Ha; the TI reference peaks at **~1.35e-9 a₀²** just above the ~0.1719 Ha threshold and decays smoothly, while **the LCP curve is drawn scaled by 1e-5** (it over-predicts by five orders). | p. 46 | `validation/diatomic/test_no_da_thesis.py` (a GATE — the ONLY published NO DA cross section, and what caught the volume-T-matrix defect in `qscat.core.dissociation.da_cross_section`) |
